...
color groen = color(#808000); //, (128, 128, 0)
color oranje = color(#FF7034); //, (255, 113, 52)
color yellow  = color(#FFDB58); //, (255, 219, 88)
color beige = color(#EBD7A8); //, (235, 215, 168)

void background_pulse()
{
  float sinv = sin(patternrow % 8 * (PI/8));
  color c  = lerpColor(color(0), bgcolor, sinv);
  fill(c);
...
{
  color c  = lerpColor(bgcolor, color(0), patternrow % 8 / 8.);
  fill(c);
  rect(width/2, height/2, 20,20); 
}
